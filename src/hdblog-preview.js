...
export function expandHdblogPreviewImages(document, locationObject = document?.location) {
  if (!document || !isHdblogHost(locationObject)) return 0;
...
export function installHdblogPreviewImages(document = globalThis.document, locationObject = globalThis.location) {
  ...
  if (!view) return;
  view.setTimeout(() => expandHdblogPreviewImages(document, locationObject), 400);
  view.setTimeout(() => expandHdblogPreviewImages(document, locationObject), 1400);
}
