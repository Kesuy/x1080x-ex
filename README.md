...
【x1080x 增强】油猴脚本：在 Discuz 帖子标题右侧增加下载按钮，一键下载主楼附件和第二张正文大图，并自动重命名。

## 功能

- 下载**主楼附件**，不会误下载回复楼层的附件。
- 下载主楼正文的**第二张图片**，优先使用 `zoomfile`、`file` 或图片链接指向的大图。
...
- 第二张主楼图片重命名为 `番号.jpg`，例如 `SNOS-325.jpg`。
...
> 首次使用 `GM_download` 时，Tampermonkey 或浏览器可能会询问下载权限，请选择允许。
...
- 大图：`zoomfile`、`file`、图片外层链接、`data-original`
