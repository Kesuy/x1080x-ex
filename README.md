...
- `agaghhh.cc` 支持**真实演员回退查询**：
  - 仅当主楼“出演者”字段为空时，从帖子标题提取番号并查询 `av-wiki.net`。
  - 查到演员后，仅在原有附件/标题型文件名末尾追加 `空格 + 演员名`；番号图片仍保持 `番号.jpg`。
  - 查询失败、超时或 av-wiki 页面结构变化时自动回退原命名，不影响原有下载。
- `agaghhh.cc` 提供独立的 **“⚙️ x1080x 设置”**，可分别开关：
  - 批量打开帖子功能
  - 下载增强
  - 查真实演员信息
- `hdblog.me` 继续使用独立的 **“⚙️ hdblog 设置”**，与 x1080x 设置互不混用。
- 在版块列表页增加 **“后台顺序打开本页主题”** 按钮：
  - 支持 Discuz 版块页与 `hdblog.me` 的 WordPress 归档、分类、标签列表页。
  - 只打开当前页主内容区的普通主题，自动排除置顶公告、侧栏文章、分类和分页链接。
  - 严格按页面从上到下的顺序在后台新标签中打开。
  - 每个主题之间随机等待 1.8–3.5 秒，每打开 8 个主题额外停顿 6–10 秒，避免瞬间集中请求。
  - `hdblog.me` 使用更快的 0.8–1.6 秒随机间隔，每 10 个主题停顿 3–5 秒。
  - 不派发可被识别为脚本生成的鼠标事件，直接使用 Tampermonkey 的正常后台标签导航。
  - 打开过程中再次点击按钮，可停止尚未打开的主题。

## 安装

1. 安装 [Tampermonkey](https://www.tampermonkey.net/)（油猴）。
2. 点击安装脚本：
   **[安装 x1080x-ex v1.8.0](https://raw.githubusercontent.com/Kesuy/x1080x-ex/v1.8.0/dist/x1080x-ex.user.js)**
...
脚本为了支持未来域名，元数据使用了 `@match *://*/*`，但业务代码只会在允许列表内的域名运行；默认仅允许 `agaghhh.cc` 和 `hdblog.me`，不会读取或上传其他网站的数据。
...
| `BLOR-306 れそう 酒とチポが大好きな塾講師おねえさん…`（出演者为空） | `BLOR-306 れそう 酒とチポが大好きな塾講師おねえさん… 桜野桃.rar` |
...
- 仅在 `agaghhh.cc` 启用“查真实演员信息”且主楼出演者为空时，请求 `av-wiki.net` 查询对应番号。
